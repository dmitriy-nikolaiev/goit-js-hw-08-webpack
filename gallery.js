...
// Create gallery
const htmlGalleryItems = galleryItems.map(({ preview, original, description: alt }) => {
...
const scrollImage = (direction) => {
  ...
  let nextImageIndex = galleryItems.findIndex(({ original }) => original === lightboxImageRef.src);
  nextImageIndex = (nextImageIndex + galleryItems.length + direction) % galleryItems.length;
...
if (currentEvent) {
  ...
  happy.style.marginTop = '200px';
  happy.style.textAlign = 'center';
